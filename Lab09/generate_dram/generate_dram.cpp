...
int main()
{
    ...
    int berry,medicine,candy,bracer,money_fir,money_sec,money_third,money_forth, stone, m1;
    ...
    for(int i=0;i<256;i++){
        for(int j=0;j<2;j++){
            ...
            if(j == 0){
                ...
                stone = rand() % 4;
                money_fir = 0;
                m1 = stone *4 + money_fir;
                money_sec = rand() % 16;
                money_third = rand() % 16;
                money_forth = rand() % 16;


                myfile << hex <<berry<<medicine<<" "<< candy<<bracer<<" "<<m1<<money_sec<<" "<<money_third<<money_forth<<endl;
            }
            else{
                stage = rand() % 3 ;
                //type = rand() % 5;
