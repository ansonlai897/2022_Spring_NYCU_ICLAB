...
// clk
// ===============================================================
wire sleep1, sleep2, sleep3, sleep4, sleep5, sleep6, sleep7;
wire clk1, clk2, clk3, clk4, clk5, clk6, clk7;

assign sleep1 = (next_state == IDLE || next_state == INPUT || next_state == OPERATION) 						  ? 0 : 1;
assign sleep2 = (next_state == RESET || next_state == OUT)    												  ? 0 : 1;
assign sleep3 = (next_state == IDLE || next_state == OPERATION) 											  ? 0 : 1;
assign sleep4 = (next_state == RESET || next_state == OPERATION || next_state == SET_OUT || next_state == OUT) ? 0 : 1;
// cnt
assign sleep5 = (next_state == IDLE || (next_state == INPUT && cnt < 15 )) ? 0 : 1;
assign sleep6 = (next_state == OPERATION || next_state == SET_OUT)  ? 0 : 1;
assign sleep7 = (next_state == SET_OUT || next_state == OUT)  ? 0 : 1;

GATED_OR GATED_CLK1(.CLOCK(clk), .SLEEP_CTRL(cg_en & sleep1), .RST_N(rst_n), .CLOCK_GATED(clk1));
GATED_OR GATED_CLK2(.CLOCK(clk), .SLEEP_CTRL(cg_en & sleep2), .RST_N(rst_n), .CLOCK_GATED(clk2));
GATED_OR GATED_CLK3(.CLOCK(clk), .SLEEP_CTRL(cg_en & sleep3), .RST_N(rst_n), .CLOCK_GATED(clk3));
GATED_OR GATED_CLK4(.CLOCK(clk), .SLEEP_CTRL(cg_en & sleep4), .RST_N(rst_n), .CLOCK_GATED(clk4));
GATED_OR GATED_CLK5(.CLOCK(clk), .SLEEP_CTRL(cg_en & sleep5), .RST_N(rst_n), .CLOCK_GATED(clk5));
GATED_OR GATED_CLK6(.CLOCK(clk), .SLEEP_CTRL(cg_en & sleep6), .RST_N(rst_n), .CLOCK_GATED(clk6));
GATED_OR GATED_CLK7(.CLOCK(clk), .SLEEP_CTRL(cg_en & sleep7), .RST_N(rst_n), .CLOCK_GATED(clk7));
// ===============================================================
// IP
// ===============================================================
